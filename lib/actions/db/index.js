...
var	util = require('util'),
	mysqlClient = require('mysql').createClient;
	...
	function getChildren(err, result) {
		...
		db.query(
			...
			function(err, data, fields) {
				if (err) {
					console.log('Error reading Album children: ', err);
	...
	function getInfo(next) {
		db.query(
			...
			function(err, data, fields) {
				...
				// get authorized users for this album
				self.getAlbumUsers(albumId,	function(err, users) {
					// error doing query
					if (err) return cb(err, users);
					// put the users in the album data if admin scope
					if (admin) {
						album.users = users;
						next(null, album);
					}
...
/**
 * Get a list of userId's connected to the given album
 * @param album {Album} A album object with a valid album.id
 * @param cb {Function} Callback function to call with (err, album) when list is available or error occured
 */
Db.prototype.getAlbumUsers = function(album, cb) {
	this.db.query(
		'SELECT * FROM albumuser ' +
		'WHERE albumId = ?',
		[album.id],
		function(err, data, fields) {
			var i, user;
			...
			// create an array of album user data
			for (i=0; user = data[i]; i++) {
 ...
 * @return {String} Used key for this new album record
 */
Db.prototype.updateAlbum = function(data, albumId, cb) {
	var db = this.db;

	console.log('db.updateAlbum', data, albumId);

	function getRecord(next) {
		db.query(
			'SELECT * FROM albums ' +
			'WHERE albumId = ?',
			[albumId],
	...
	function setData(err, album) {
		// data from requested record not found so return error
		if (err) return cb(err, album);
		
		// merge given data with retrieved record
		// merge(album, data);
	...
	// first get current data and then put merged data
	getRecord(setData);
}
